...
For challenge/captcha provider adapter guidance, including acceptable-use
requirements and a harmless mock provider, see
[docs/captcha-provider-adapters.md](docs/captcha-provider-adapters.md).
...
| Extension surfaces | Credential metadata/autofill, audio capture, challenge governance, Sentinel signals, and mobile bridge hooks. The stock public build returns unavailable unless an extension provider is attached; credential, challenge, and URL metadata/errors are redacted at the MCP boundary. See the captcha adapter guide for the BYO provider pattern. |
