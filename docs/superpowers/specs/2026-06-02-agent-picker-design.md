...
Add a `last_selected_at INTEGER DEFAULT 0` column to the `agents` table.
The value is a Unix timestamp in seconds; `0` means the agent has never
been selected. Migration runs in the same place as the existing schema
migrations via the idempotent column helper so it only runs when the
column is missing.
