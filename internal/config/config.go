...
var Providers = []Provider{
	...
	{ID: "opencode", Name: "OpenCode (Zen)", EnvVar: "OPENCODE_API_KEY",
		...
		Models:       []string{"opencode/deepseek-v4-flash-free", "opencode/minimax-m2.5", "opencode/claude-opus-4-6", "opencode/claude-sonnet-4-6"},
		NeedsKey:     true},
	{ID: "opencode-go", Name: "OpenCode Go", EnvVar: "OPENCODE_API_KEY",
		DefaultModel: "opencode-go/minimax-m2.5",
		Models:       []string{"opencode-go/minimax-m2.5", "opencode-go/deepseek-v4-flash", "opencode-go/deepseek-v4-pro", "opencode-go/minimax-m2.7"},
		NeedsKey:     true},
...
// RefreshSetupComplete recomputes whether the config is complete enough to skip
// setup: a known provider plus a model, and an API key when the provider needs
// one.
func (c *Config) RefreshSetupComplete() bool {
	c.SetupComplete = c.configReady()
	return c.SetupComplete
}

// configReady reports whether the active provider/model/key are sufficient to
// run agents.
func (c *Config) configReady() bool {
	if strings.TrimSpace(c.Provider) == "" || strings.TrimSpace(c.Model) == "" {
		return false
	}
	provider := GetProvider(strings.TrimSpace(c.Provider))
	if provider == nil {
		return false
	}
