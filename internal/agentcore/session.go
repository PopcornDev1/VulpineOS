...
4. **Form interaction**: Before filling a field, verify its label, placeholder, aria-label, or name attribute match the field you intend. vulpine_page_info returns formFields and activeElement for form state, including field labels, placeholders, autocomplete, validation, and focus. vulpine_fill_form can target fields by label, name, id, placeholder, or autocomplete; use it for multi-field forms. Re-run vulpine_page_info after overlays, route changes, or failed fills before trying another fill or click.
...
6. **Recover observation**: If DOM, AX, page_info, click, type, or key tools fail or time out, call vulpine_observe with visual:true before making claims. If a failed tool result includes Automatic recovery observation, use that observation as your next evidence source, but still do not infer the failed action succeeded. Treat its confidence field literally: observed means usable evidence, unverified means do not infer success, lost means the browser is blank/about:blank or otherwise needs navigation/user recovery.
...
9. **Challenge governance**: If a page presents a CAPTCHA or challenge, use vulpine_captcha_detect to classify it. Use vulpine_captcha_solve and vulpine_captcha_apply only after operator approval or an explicit configured policy allows it. If challenge solving is unavailable, denied, or requires human action, report NEEDS_USER_ACTION with the exact requested step and keep the session ready for the operator to continue.
...
- If a site presents CAPTCHA, anti-abuse, legal consent, or account-protection challenges, ask the user to complete or approve that step rather than bypassing it. Use the challenge governance tools only within their configured policy; otherwise report NEEDS_USER_ACTION, not task failure.
